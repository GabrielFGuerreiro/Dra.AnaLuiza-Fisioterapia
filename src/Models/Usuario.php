...
class Usuario
{
    ...
    public function CadastrarPreConsulta(string $email, string $dtPreferencia, string $horarioInicial, string $horarioFinal, string $observacao = ''): array
    {
        try
        {
            ...
            $sql = "INSERT INTO PreConsultas (idUsuario, dtConsulta, horarioInicial, horarioFinal, observacao, aceito)
                    VALUES (:idUsuario, :dtConsulta, :horarioInicial, :horarioFinal, :observacao, null)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                ':idUsuario' => $idUsuario,
